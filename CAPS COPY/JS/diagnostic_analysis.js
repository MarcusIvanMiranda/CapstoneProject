// diagnostic_graph.js (Dedicated to Heartbeat Graph)
...
const API = { 
    ...
    // Heartbeat API endpoint for the graph data.
    heartbeat: {
...
// --- HEARTBEAT LOGIC (Updated to remove comparison) ---
 ...
 * Renamed from loadCurrentLogs to loadLogs to reflect singular functionality.
 */
...
document.addEventListener('DOMContentLoaded', () => {
    // Only one listener is needed for the date input.
    $('#diagnostic-date').addEventListener('change', loadLogs);
    // Both time-range and view-mode trigger the single log load.
    $('#time-range-select').addEventListener('change', loadLogs);
    $('#view-mode-select').addEventListener('change', loadLogs);
});


// --- INITIAL PAGE LOAD (Updated to remove comparison logic) ---
...
function renderCatDetails(cat){
    ...
    // Set default dates and dropdown values for logs (Comparison date is removed).
...
async function loadCatData(){
    ...
    // Initial load of the single log container
    loadLogs(); 
}
